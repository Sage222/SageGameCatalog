...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        favorites = sum(1 for g in self.games.values() if g.favorite)
        self.log(f"Archive catalog ready. {len(self.games)} game(s) in catalog ({favorites} favorite(s)).")
    ...
    def import_library_folder(self, folder_path: str):
        ...
        # Remove entries whose folders no longer exist under this root
        scanned_ids = {
            self.generate_game_id(str(child))
        ...
        for game_id in list(self.games.keys()):
            ...
            if Path(game.path).parent.resolve() == root.resolve():
                if game_id not in scanned_ids:
                    del self.games[game_id]
                    removed_stale += 1
        if removed_stale:
            self.log(f"Removed {removed_stale} stale entr{'y' if removed_stale == 1 else 'ies'} no longer on disk.")

        if added == 0 and updated == 0 and removed_stale == 0:
            skipped = 1

        self.save_all()
        self.refresh_ui()
        self.log(f"Import complete. Added {added}, updated {updated}, removed {removed_stale}, skipped {skipped}.")
        favorites = sum(1 for g in self.games.values() if g.favorite)
        self.log(f"Catalog total: {len(self.games)} game(s) ({favorites} favorite(s)).")
